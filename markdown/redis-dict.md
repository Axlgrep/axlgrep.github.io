...
### 总结
Redis的字典实现非常的巧妙，利用两个哈希表渐进式rehash的方法在解决单个HashTable在保存结点过多的场景下查询/更新/删除效率低下和单个大型HashTable在保存少量结点场景下浪费内存问题的同时不影响服务的正常运行，并且通过构造字典时传入的type和privdata实现了多态字典，用户可以根据自身数据类型的不同设置相应的type和privdata来使用字典，大大的提高了